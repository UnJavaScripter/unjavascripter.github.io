...
Ten en cuenta que la práctica más común es usar los guiones bajos (`_`) para representar cursiva, el asterísco (`*`) se suele usar para escribir texto en negrita/negrilla.
...
Podemos agregar o quitar cuántos elementos querramos y no tenemos que cambiar los números de línea a mano (muy útil).
...
### Listas anidadas y combinaciónes

Podemos anidar elementos de lista poniéndo agregando espacios, uno es suficiente pero se ve mejor en el código si trabajamos con dos espacios:
...
Partimos de un lik y agregamos un `!` al principio:
